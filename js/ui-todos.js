...
CREATE POLICY "auth_only" ON todos
  ...
  const filtered = todos.filter((t) =>
    TODO_FILTER === 'all' ? true : TODO_FILTER === 'done' ? t.done : !t.done,
  );
...
async function toggleTodoDone(id) {
  ...
  const t = TODOS_DATA.find((x) => x.id === id);
  if (!t) return;
...
function getDismissed() {
  try {
    return new Set(JSON.parse(localStorage.getItem('sp_dismissed') || '[]'));
  } catch {
    return new Set();
  }
}
