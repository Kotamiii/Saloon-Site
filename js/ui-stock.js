...
function renderStock() {
  if (STOCK_DATA === undefined) {
    $('#view').innerHTML =
      head('Stock') + `<p class="note" style="text-align:center;padding:20px">Chargement du stock…</p>`;
    loadStock().then(() => {
...
CREATE POLICY "auth_only" ON stock
  ...
  $('#view').innerHTML =
    head('Stock') +
    `<p class="note">Épuisé = 0 · Faible ≤ 3 · Modifie les quantités en direct · <b>+ Production</b> = ajouter des unités craftées au stock.</p>` +
    html;
}
function askInput(msg, placeholder, onConfirm, confirmLabel = 'Confirmer') {
  ...
  el.querySelector('#_askY').onclick = () => {
    const v = el.querySelector('#_askVal').value;
    close();
    onConfirm(v);
  };
  ...
  el.querySelector('#_askVal').addEventListener('keydown', (e) => {
    if (e.key === 'Enter') el.querySelector('#_askY').click();
  });
...
async function productionToStock(produit, currentQty) {
  askInput(
    ...
    async (val) => {
      ...
      toast(
        `+${qte} unité${qte > 1 ? 's' : ''} ajoutée${qte > 1 ? 's' : ''} · ${produit} : stock = ${newQty}`,
      );
...
async function updStock(produit, valeur) {
  ...
  const { error } = await sb
    .from('stock')
    .upsert(
      { produit, quantite: qty, updated_at: new Date().toISOString(), updated_by: who },
      { onConflict: 'produit' },
    );
  if (error) {
    toast('Erreur : ' + error.message, 'err');
